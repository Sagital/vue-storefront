...
export interface UseCartFactoryParams<CART, CART_ITEM, PRODUCT, COUPON>
  ...
  removeItem: (
    context: Context,
    params: { currentCart: CART; product: CART_ITEM; customQuery?: CustomQuery }
  ) => Promise<CART>;
  updateItemQty: (
    context: Context,
    params: {
  ...
  isInCart: (
    context: Context,
    params: { currentCart: CART; product: PRODUCT }
  ) => boolean;
}
  ...
  return function useCart(): UseCart<CART, CART_ITEM, PRODUCT, COUPON> {
    ...
    const load = async ({ customQuery } = { customQuery: undefined }) => {
      ...
      try {
        loading.value = true;
        error.value.load = null;
        console.log('loading cart in useCartFactory');
    ...
    const clear = async () => {
      ...
      try {
        ...
        const updatedCart = await _factoryParams.clear({
          currentCart: cart.value
        });
        cart.value = updatedCart;
      } catch (err) {
    ...
    const computedCart: ComputedRef<CART> = computed(() => cart.value);
